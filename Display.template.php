...
function template_main()
{
	...
	if ($context['is_poll'])
	{
		...
		$poll_buttons = array(
			...
			'votelog' => array('test' => 'show_view_votelog_button', 'text' => 'poll_vv_vlog_button', 'image' => 'poll_votelog.gif', 'lang' => true, 'url' => $scripturl . '?action=votelog;topic=' . $context['current_topic']),
	...
	while ($message = $context['get_message']())
	{
		...
		// Show banned message in topics & posts
			if ($message['member']['is_banned'])
				echo '
								<li style="background-color: #c0392b;color: white;padding: 2px;" class="banned-placa">', $txt['banned'], '</li>';
		...
		if (!$message['member']['is_guest'])
		{
				...
				echo '
								<li style="background-color: ', $message['member']['group_color'], ';color: white;padding: 2px;" class="placa">', $message['member']['group'], '</li>';
			...
			if (!empty($message['member']['additional_groups_full'])) {
				foreach($message['member']['additional_groups_full'] as $temp) {
					...
					echo '
									<li style="background-color: ', $temp['group_color'], ';color: white;padding: 2px;margin-top: 5px;" class="placa">', $temp['group'], '</li>';
				}
			}
           echo '
           ...
           <hr></hr>';
			...
			if ((empty($settings['hide_post_group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
				echo '
								<li style="background-color: #ccc;color: white;padding: 2px;" class="postgroup">', $message['member']['post_group'], '</li>';
			...
			if (!empty($settings['show_gender']) && $message['member']['gender']['text'] != '' && !isset($context['disabled_fields']['gender']))
				echo '
								<li class="gender">', $txt['gender'], ': ', $message['member']['gender']['text'], '</li>';
			...
			if (!empty($message['member']['custom_fields']))
			{
				...
				foreach ($message['member']['custom_fields'] as $custom)
				{
					...
					echo '
										<li style="color: white;background-color: #BDBDBD;padding: 2px;margin-bottom: 10px;">', $custom['value'], '</li>';
				}
				if ($shown)
					echo '
		...
		echo '
				...
				if ($message['can_approve'] || $context['can_reply'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'])
					echo '				
										<div class="btn-group navbar-right">
											<button type="button" class="btn btn-primary dropdown-toggle btn-sm" data-toggle="dropdown">Acciones <span class="caret"></span></button>
				...
				echo '
										<div class="inner" id="msg_', $message['id'], '"', '>', $message['id'] == $context['first_message'] ? template_ad_position('inside_first_post') : '', $message['body'], '</div>
									</div>';
